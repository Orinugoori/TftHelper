package com.orinugoori.tfthelper.repository
...
import com.orinugoori.tfthelper.constants.AppConstants.CACHE_DURATION_MS
import com.orinugoori.tfthelper.constants.AppConstants.EXCLUDED_AUGMENT_NAMES
import com.orinugoori.tfthelper.constants.AppConstants.KEY_CACHED_AUGMENTS
import com.orinugoori.tfthelper.constants.AppConstants.KEY_CACHED_VERSION
import com.orinugoori.tfthelper.constants.AppConstants.KEY_CACHE_TIMESTAMP
import com.orinugoori.tfthelper.data.model.Augment
import com.orinugoori.tfthelper.data.remote.client.RetrofitInstance
import com.orinugoori.tfthelper.util.DebugUtils.analyzeApiDataForDebugging
import com.orinugoori.tfthelper.util.DebugUtils.logAugmentData
import com.orinugoori.tfthelper.util.DebugUtils.logAugmentDataGrouped
import com.orinugoori.tfthelper.util.DebugUtils.logSpecificPatterns
import com.orinugoori.tfthelper.util.DebugUtils.verifyBlacklistedAugments
import com.orinugoori.tfthelper.util.processAugmentData
...
class AugmentRepository(private val context: Context) {
    ...
    private fun filterValidAugments(augments: List<Augment>): List<Augment> {
        ...
        val filteredAugments = augments.filter { augment ->
            val imageUrl= augment.imageUrl.lowercase()
            val augmentName = augment.name

            // 🚫 제외 조건들
            val isExcluded = when {
                // 1. 이미지명에 tutorial 포함된 경우
                imageUrl.contains("tutorial") -> {
                    Log.d("AugmentRepository", "🚫 Tutorial 제외: ${augment.name} (${augment.imageUrl})")
                    true
                }
                // 2. 이미지명에 revival 포함된 경우
                imageUrl.contains("revival") -> {
                    Log.d("AugmentRepository", "🚫 Revival 제외: ${augment.name} (${augment.imageUrl})")
                    true
                }
                ...
                imageUrl.contains("set10") -> {
                    Log.d("AugmentRepository", "🚫 Set10 제외: ${augment.name} (${augment.imageUrl})")
                    true
                }
                else -> false
            }
